...
B as(
SELECT ID, Max(FUNC_DATE) AS F_D_DATE FROM A
GROUP BY ID
)
select B.ID, substring(B.F_D_DATE,1,4) as last_year, HIGH_AGE_ID.dbo.UID100W.ID_SEX, HIGH_AGE_ID.dbo.UID100W.NEW_BIRTHDAY,
death_all.death_year
into HIGH_AGE_CD.dbo.LAST_DIABETES_05_13 from B
left join HIGH_AGE_ID.dbo.UID100W
on HIGH_AGE_ID.dbo.UID100W.ID = B.ID
left join HIGH_AGE_ID.dbo.death_all
on HIGH_AGE_ID.dbo.death_all.id = B.ID